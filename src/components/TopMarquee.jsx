...
const TopMarquee = () => {
  ...
  return (
    <div className={`fixed top-0 left-0 right-0 z-50 h-12 sm:h-14 bg-primary-dark border-b border-white/5 flex items-center overflow-hidden transition-all duration-500 ${isScrolled ? '-translate-y-full opacity-0' : 'translate-y-0 opacity-100'
      }`}>
      <div className="max-w-7xl mx-auto px-6 md:px-12 w-full flex items-center justify-between">
        ...
        <div className="shrink-0 flex items-center gap-2 pr-4 bg-primary-dark z-10 shadow-[4px_0_8px_rgba(11,31,58,0.95)]">
          ...
          <span className="text-[10px] sm:text-[11px] font-bold tracking-[0.2em] text-[#C8A96A] font-sans uppercase">
            Trusted By:
          </span>
        </div>
        ...
        <div className="w-full overflow-hidden relative flex items-center">
          ...
          <div className="animate-marquee flex gap-4 sm:gap-6 items-center whitespace-nowrap">
            {marqueeList.map((logoPath, idx) => (
              <div
                key={idx}
                className="flex items-center justify-center h-8 sm:h-10 px-2 sm:px-3 bg-white rounded border border-black/5 shadow-[0_2px_6px_rgba(0,0,0,0.03)] min-w-[55px] sm:min-w-[90px] hover:scale-105 transition-transform duration-300"
              >
                <img
                  src={logoPath}
                  alt={`Partner Logo ${idx + 1}`}
                  className="h-5.5 sm:h-7 w-auto object-contain pointer-events-none"
                />
              </div>
            ))}
          </div>
